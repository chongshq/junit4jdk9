...
public class RuleChainTest {
    ...
    public static class UseRuleChain {
        @Rule
        public final RuleChain chain = RuleChain.outerRule(new LoggingRule("outer rule"))
    ...
    public void executeRulesInCorrectOrder() throws Exception {
        PrintableResult.testResult(UseRuleChain.class);
    ...
    public void whenRuleChainHasNullRuleTheStacktraceShouldPointToIt() {
        ...
        MatcherAssert.assertThat(result.getFailures().size(), equalTo(1));
        String stacktrace = Throwables.getStacktrace(result.getFailures().get(0).getException());
        assertThat(stacktrace, containsString("\tat RuleChainTest$RuleChainWithNullRules.<init>(RuleChainTest.java:"));
    }
}