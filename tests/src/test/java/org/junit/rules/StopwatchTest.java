...
public class StopwatchTest {
    ...
    public static class DurationDuringTestTest extends AbstractStopwatchTest {
        ...
        public void duration() {
            ...
            Assert.assertEquals(300L, stopwatch.runtime(MILLISECONDS));
            simulateTimePassing(500L);
            Assert.assertEquals(800L, stopwatch.runtime(MILLISECONDS));
        }
    }
    ...
    public void succeeded() {
        ...
        Assert.assertThat(record.name, is("successfulTest"));
        Assert.assertThat(record.name, is(finishedRecord.name));
        Assert.assertThat(record.status, is(TestStatus.SUCCEEDED));
        Assert.assertTrue("timeSpent > 0", record.duration > 0);
        Assert.assertThat(record.duration, is(finishedRecord.duration));
    }
    ...
    public void failed() {
        ...
        Assert.assertThat(record.name, is("failedTest"));
        Assert.assertThat(record.name, is(finishedRecord.name));
        Assert.assertThat(record.status, is(TestStatus.FAILED));
        Assert.assertTrue("timeSpent > 0", record.duration > 0);
        Assert.assertThat(record.duration, is(finishedRecord.duration));
    }
    ...
    public void skipped() {
        ...
        Assert.assertEquals(0, result.getFailureCount());
        Assert.assertThat(record.name, is("skippedTest"));
        Assert.assertThat(record.name, is(finishedRecord.name));
        Assert.assertThat(record.status, is(TestStatus.SKIPPED));
        Assert.assertTrue("timeSpent > 0", record.duration > 0);
        Assert.assertThat(record.duration, is(finishedRecord.duration));
    }
