...
public final class ConcurrentRunNotifierTest {
    ...
    public void realUsage() throws Exception {
        ...
        pool.shutdown();
        Assert.assertTrue(pool.awaitTermination(TIMEOUT, TimeUnit.SECONDS));

        fNotifier.removeListener(listener1);
        fNotifier.removeListener(listener2);

        Assert.assertThat(listener1.fTestStarted.get(), is(numParallelTests));
        Assert.assertThat(listener2.fTestStarted.get(), is(numParallelTests));
    }
    ...
    private abstract class AbstractConcurrentFailuresTest {
        ...
        public void test() throws Exception {
            ...
            condition.set(false);
            Assert.assertTrue(notificationsPool.awaitTermination(TIMEOUT, TimeUnit.SECONDS));
            ...
            if (totalListenersFailures != 0) {
                ...
                Assert.assertThat(totalListenersFailures, is(countTestFailures));
            }
        }
    }
