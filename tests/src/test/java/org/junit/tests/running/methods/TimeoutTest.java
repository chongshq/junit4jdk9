...
public class TimeoutTest {
    ...
    public void infiniteLoopRunsForApproximatelyLengthOfTimeout() throws Exception {
        ...
        Assert.assertTrue(String.format("Difference was %sms", difference), difference < 200);
    }
    ...
    public void stalledThreadAppearsInStackTrace() throws Exception {
        ...
        Assert.assertEquals(1, result.getFailureCount());
        Throwable exception = result.getFailures().get(0).getException();
        Assert.assertThat(stackForException(exception), containsString("infiniteLoop")); // Make sure we have the stalled frame on the stack somewhere
    }
    ...
    public void timeoutFailureMultithreaded() throws Exception {
        ...
        Assert.assertThat(exception[0].getMessage(), containsString("test timed out after 100 milliseconds"));
        Assert.assertThat(stackForException(exception[0]), containsString("Thread.join"));
        Assert.assertThat(exception[1].getMessage(), containsString("Appears to be stuck in thread timeout-thr2"));
    }
    ...
    public void timeoutFailureMultithreadedStuckInMain() throws Exception {
        ...
        Assert.assertEquals(1, result.getFailureCount());
        Throwable exception = result.getFailures().get(0).getException();
        Assert.assertThat(exception.getMessage(), containsString("test timed out after 100 milliseconds"));
        Assert.assertThat(exception.getMessage(), not(containsString("Appears to be stuck")));
    }
    ...
    public void makeSureAfterIsCalledAfterATimeout() {
        ...
        Assert.assertThat(WillTimeOut.afterWasCalled, is(true));
    }
